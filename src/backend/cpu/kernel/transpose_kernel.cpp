...
void transpose2d_tiled(const unsigned char* src_raw, unsigned char* dst_raw, int rows, int cols)
{
    ...
    const long long row_blocks = static_cast<long long>((rows + TILE - 1) / TILE);
    ...
    auto process_row_blocks = [&](long long begin_block, long long end_block) {
        for (long long block_idx = begin_block; block_idx < end_block; ++block_idx)
        {
            const int row0 = static_cast<int>(block_idx) * TILE;
    ...
    if (parallel)
    {
        parallel_for_1d(0, row_blocks, 1, process_row_blocks);
    }
    else
    {
        process_row_blocks(0, row_blocks);
    }
}
