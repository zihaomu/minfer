...
#ifndef MINFER_MAT_EXPR_H
#define MINFER_MAT_EXPR_H
...
static void checkOperandsExist(const Mat& a)
{
    if (a.empty())
    {
        assert(0 && "Matrix operand is an empty matrix!");
    }
}
...
static void checkOperandsExist(const Mat& a, const Mat& b)
{
    if (a.empty() || b.empty())
    {
        assert(0 && "One or more matrix operands are empty!");
    }
}
...
void MatOp_Identity::assign(const MatExpr &e, Mat &m, int _type) const
{
    if (_type == -1 || _type == e.a.type())
        ...
        assert(0 && "Unsupported data type convert!");
}
